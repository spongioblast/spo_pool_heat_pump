...
1. **Serial Setting:** **9600 8N1**, CTSRTS Disable, Pack Interval **10** (ms; the firmware minimum — factory is 20, and 10 shaves ~10 ms off every reply), Pack Size **1400**, Com Heart **OFF**, ModBUS Enabled **OFF**. The HA framer accepts split frames.
...
| Writes pulse, then snap back after 12 s | Log shows `write … not confirmed`. Check the write path under Configure is **Second panel (slave 2)**; slave 99 is mode-only on this bus. Mode/timers wait 20 s. Through-the-house WiFi can miss the board's 340 ms page-read deadline — Pack Interval 10, `AT+EVENT=off`, then Ethernet if it persists. Dump-only never writes |
