...
- **Lost page reply no longer loses the write.** The slave 2 responder keeps its 3011 bit raised until the board's 3001 sync acknowledges the read (flags 0); when the board echoes the bit instead — it never got our page and re-read the wired display's — the next poll makes it read again. Previously the bit was cleared on send, so 1 in 12 mode changes on the live bus silently fell back to the display's value. On the slave 2 path Home Assistant also no longer issues its own FC03 reads of the display after a flag change (the board pushes every changed page); the board's sync to slave 2 used to be mistaken for a display flag change and triggered one
