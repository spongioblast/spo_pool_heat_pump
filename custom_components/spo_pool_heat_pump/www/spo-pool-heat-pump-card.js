...
var PoolHeatPumpCard = class extends i4 {
  ...
  view() {
    ...
    return {
      ...
      pending: Array.isArray(climate.attributes.pending_writes) ? climate.attributes.pending_writes : [],
      caps
    };
  }
  ...
  render() {
    ...
    const pend = (...keys) => keys.some((k2) => s4.pending.includes(k2)) ? "pending" : "";
    return b2`
        ...
        <div class="w3 ${schematic}">
          ...
          <div class="hero">
            ...
            ${s4.dumpOnly ? b2`<div class="target"><span class="eyebrow">No map</span><span class="val">Use Settings → Bus dump</span></div>` : b2`<div class="target">
              ...
              <span class="val ${this._targetLocal != null ? "pending" : pend("setpoint")}"><span class="num">${f1(s4.setpoint)}</span><span class="unit">°</span></span>
          ...
          <div class="modes">
            ${s4.dumpOnly ? "" : b2`<div class="seg">${modes.map((m2) => b2`<button class="${s4.power && s4.mode === m2 ? "on" : ""} ${s4.mode === m2 ? pend("mode", "power") : ""}" @click=${() => this.setMode(m2)}>${m2 === "heat" ? "Heat" : m2 === "cool" ? "Cool" : "Auto"}</button>`)}</div>`}
            <span class="sp"></span>
            ${showTune ? b2`<button class="ib params-open" title="Heat pump settings" @click=${() => this.openSettings()}>${TUNE}</button>` : ""}
            ${s4.dumpOnly || !s4.caps.silent ? "" : b2`<button class="ib quiet ${s4.silent ? "on" : ""} ${pend("silent")}" title="Quiet mode"
...
var DOCS_URL = "https://github.com/spongioblast/spo_pool_heat_pump/blob/main/README.md";
var DEFINE_FALLBACK_MS = 5e3;
function ourClimate(hass, entityId) {
...
function getSettingsEntitySuggestion(hass, entityId) {
  if (!ourClimate(hass, entityId)) return null;
  return {
    config: {
